    ...
    <div className="bg-card border border-border rounded-xl p-4 hover:shadow-md transition-all active:scale-98">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          ...
          {/* Package Name */}
          <h3 className="text-sm font-semibold text-foreground mb-2">{pkg.name}</h3>

          {/* Resources */}
          <p className="text-xs text-muted-foreground mb-3">{getResourcesText()}</p>

          {/* Badge */}
          {pkg.badge && (
            <p className="text-xs font-medium text-green-600 bg-green-50 px-2 py-1 rounded w-fit mb-3">{pkg.badge}</p>
          )}
        ...
        <div className="flex flex-col items-end gap-3">
          ...
          {/* Popular Badge */}
          {pkg.popular && (
            <span className="px-2 py-1 bg-accent text-accent-foreground text-xs font-semibold rounded-lg">Popular</span>
          )}
